...
class MongodbStore implements StoreInterface
{
    ...
    public function get($key)
    {
        ...
        if (!is_null($cache)) {
            if (class_exists('MongoDate')) {
                ...
                if (time() * 1000 >= intval((string)($cache['expiration']))) {
                    return $this->forget($key);
                }
            }
    ...
    public function put($key, $value, $minutes)
    {
        ...
        if (!class_exists('MongoDate')) {
            $expiration = new UTCDatetime((strtotime(date(DATE_ISO8601, time())) + ($minutes * 60)) * 1000);
        } else {
            $expiration = new \MongoDate($this->getTime() + ($minutes * 60));
        }

        $data = ['expiration' => $expiration, 'key' => $key, 'value' => $value];
